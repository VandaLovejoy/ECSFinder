...
typedef struct {
  ...
  int   length;
  sect  *struct_list;
  sect  *reactive;  /* 'rain' falling from the sky */
  int   reactive_n;
  sect  *probe;     /* 'probes' shot into the solution */
  int   probe_n;
  sectf *fragments; /* 'fragments' of the 'probe' injector */
  int   fragments_n;
} bt_data;


/* convert nucleotide to color */
PRIVATE INLINE int
n2c(char n)
{
  switch (n) {
...
updateFragments(bt_data *d)
{
  ...
  while ((*list).ml >= 0) {
    ...
    /* first calc the new location */
    (*list).x += (*list).vel[0];
    (*list).y += (*list).vel[1];
    /* now update the velocity vector */
    (*list).vel[1] -= gravity;
    /* remove if it disappears below surface */
...
sanitize_input(const char *string)
{
  ...
  int             inv_bb[3]; /* [0]...x_min, [1]...x_max, [2]...y_min */
  float           score;
  ...
  hpos      = 50;
  cnt2      = 0;
  inv_bb[2] = 10;     /* initial y_min of invader */
  direction = 1;
  stage     = 1;      /* current level */
  attempts  = 3;      /* current number of attempts */
  probe_max = 3;      /* maximum allowd probes on the grid */
  score     = 0;
  ...
  while (1) {
    ...
    switch (state) {
      ...
      case 2:
        if (!cnt2) {
            ...
            if (insideBB(i, j, inv_bb)) {
              ...
              while ((*(d->struct_list)).ml >= 0) {
                ...
                if ((i == p) && (j == q)) {
                  ...
                  /* recalculate structure and add score */
                  d->structure =
          ...
          fflush(stdout);
          getBoundary(d->struct_list, inv_bb);
          /* check for collission of reactive with cannon */
          ts1 = d->reactive;
          while ((*(d->reactive)).ml >= 0) {
            int i = (*(d->reactive)).i;
            int j = (*(d->reactive)).j;
    ...
    switch (c) {
      ...
      case 27:
        ...
        /* otherwise process the escaped key press */
        if (c == 91) {
          read(0, &c, sizeof(char));
          if (state == 2) {
    ...
    if (d) {
      /* display reactive in case we are hit */
      ts1 = d->reactive;
      while ((*(d->reactive)).ml >= 0) {
        int i = (*(d->reactive)).i;
        int j = (*(d->reactive)).j;
